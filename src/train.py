...
logger = setup_logger(name="train_logger")
...
def train(epochs=EPOCHS, lr=LR, batch_size=BATCH_SIZE):
    ...
    train_loader, val_loader, classes = get_loaders(batch_Size=batch_size)

    model = ASLCNNModel(num_classes=len(classes)).to(DEVICE)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=lr)
    scheduler = ReduceLROnPlateau(optimizer, mode="min", patience=2, factor=0.5)

    checkpoint_path = MODELS_DIR / "asl_cnn_model.pth"
    start_epoch = 0
